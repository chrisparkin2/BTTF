//
//  UIColor+Extensions.m
//  bttf
//
//  Created by Admin on 2/25/15.
//  Copyright (c) 2015 bttf. All rights reserved.
//


#import "UIColor+Extensions.h"

@implementation UIColor (Extensions)


+ (UIColor *)colorMustard {
    return [UIColor colorWithRed:204/255.0 green:165/255.0 blue:28/255.0 alpha:1];
}

+ (UIColor *)colorSalmon {
    return [UIColor colorWithRed:255/255.0 green:92/255.0 blue:73/255.0 alpha:1];
}

+ (UIColor *)colorBrightRed {
    return [UIColor colorWithRed:238/255.0 green:66/255.0 blue:77/255.0 alpha:1];
}

+ (UIColor *)colorFog {
    return [UIColor colorWithRed:238/255.0 green:234/255.0 blue:230/255.0 alpha:1];
}

@end
